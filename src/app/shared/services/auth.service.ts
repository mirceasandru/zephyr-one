...
export class AuthService {
    ...
    private readonly gdprod_at = "gdprod_at";
    private readonly gdprod_rt = "gdprod_rt";
    ...
    getAccessToken() {
        return localStorage.getItem(this.gdprod_at);
    }

    private getRefreshToken() {
        return localStorage.getItem(this.gdprod_rt);
    }
    ...
    private storeTokens(tokens) {
        localStorage.setItem(this.gdprod_at, tokens.access_token);
        localStorage.setItem(this.gdprod_rt, tokens.refresh_token);
    }

    private removeTokens() {
        localStorage.removeItem(this.gdprod_at);
        localStorage.removeItem(this.gdprod_rt);
    }

    verifyRank(user, target) {
        if (user.role.name === 'super') {
            return true;
        } else {
            if (target.role.rank > user.role.rank) {
                return true;
            } else {
                return false;
            }
